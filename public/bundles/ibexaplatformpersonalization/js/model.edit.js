(function(doc, Routing, eZ, Translator, React, ReactDOM) {
    ...
    const onConfirm = (event, items) => {
        ...
        items.forEach((item) => {
            const numberOfItems = editorialModelsContainer.querySelectorAll(SELECTOR_EDITORIAL_ITEM).length;
            ...
            if (!alreadySelectedItem) {
                editorialModelsContainer.insertAdjacentHTML(
                    'beforeend',
                    template.replace('__label_name__', contentName).replaceAll('__name__', numberOfItems)
                );
